...
export const Register = async (req, res) => {
  ...
  if (!fullName || !email || !password || !role) {
    ...
    !password.match(/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{5,}$/)
  ) {
    res
      .status(404)
      .json({
  ...
  try {
    ...
    if (!user)
      res
        .status(404)
        .json({ message: "failed to create the user", success: false });

    // const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1h' });
    res
      .status(201)
      .json({ message: "Register successfully", success: true, user });
  } catch (error) {
    res
      .status(404)
      .json({
...
export const login = async (req, res) => {
  ...
  if (
    password.length < 5 ||
    !/(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{5,}/.test(password)
  ) {
    return res
      .status(400)
      .json({
        message:
          "Password must be at least 5 characters, contain 1 special character and 1 number",
        success: false,
      });
  }
